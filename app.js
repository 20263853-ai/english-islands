...
(function() {
  ...
  function speak(text, idx, onEnd) {
    ...
    // 优先使用预生成的高质量MP3音频
    var audioDir = state.user === 'mumu' ? 'audio/child' : 'audio/adult';
    var audioFile = audioDir + '/' + state.islandIdx + '_' + String(idx).padStart(3,'0') + '.mp3';
    var audio = new Audio(audioFile);
    utterance = audio;
    audio.onended = function(){ state.playingIdx=null; updatePlayingUI(); if(onEnd) onEnd(); };
    audio.onerror = function(){
      // MP3加载失败时回退到SpeechSynthesis
      state.playingIdx=null; updatePlayingUI();
      speakFallback(text, idx, onEnd);
    };
  ...
  function speakFallback(text, idx, onEnd) {
    if(!synth) return;
  ...
  function renderDetail() {
    ...
    isl.sentences.forEach(function(s, idx) {
      ...
      var imgTag = isChild() ? '<img src="img/child/' + ii + '_' + String(idx).padStart(3,'0') + '.jpg" class="w-full rounded-lg mb-2" alt="" loading="lazy" onerror="this.style.display='none'">' : '';
